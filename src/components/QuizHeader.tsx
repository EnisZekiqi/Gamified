  ...
  return (
    <header className="bg-white text-black flex items-center justify-between w-full p-4 shadow-sm">
      <div className="flex items-center justify-between w-full gap-4 px-4">
        {/* Player Info */}
        <Link to="/users" className="block sm:hidden">
          <CgUser size={25} />
        </Link>
      ...
      <AnimatePresence>
        {congrats && (
          ...
          >
            <div
              className="fixed bg-black/60 top-0 right-0 left-0 bottom-0 w-full h-full "
            ...
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-8 rounded-lg shadow-lg z-50 w-96">
              <div className="p-8 bg-blue-200 border border-[#2563eb]/40 text-[#2563eb] rounded">
                <h2 className="text-2xl font-bold mb-4">Congrats </h2>
                <p className="text-lg">
                  You've advanced to <b>{rank}</b> rank!
                </p>
              </div>
            </div>
          </motion.div>
