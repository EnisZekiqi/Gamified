...
function QuizDetail() {
  ...
  if (isLoading) return
  ;<div className="p-7 h-[100vh] overflow-y-auto flex flex-col items-center justify-between w-full">
    <h1 className="text-[18px] sm:text-2xl font-semibold mb-12 w-full text-start animate-pulse rounded-full bg-gray-400"></h1>

    <div className="mb-6 p-4 flex flex-col items-center w-full justify-center ">
      <h2 className="text-[16px] sm:text-2xl font-medium mb-2 w-full animate-pulse rounded-full bg-gray-400"></h2>
      ...
      <ul className="grid grid-cols-2 gap-4 list-disc list-inside items-start justify-center w-full mt-8">
        {Array.from({ length: 4 }).map((_, ansIndex) => (
          ...
          >
            <label className="flex items-center justify-center gap-2 cursor-pointer text-sm sm:text-lg">
              <input
                type="radio"
                ...
                disabled
              />
              <div className="w-full h-6 bg-gray-400 rounded-full animate-pulse"></div>
            </label>
          </li>
        ))}
      </ul>
    ...
    <div className="flex items-center justify-between w-full">
      <button>
        <Link to="/quiz">Back to other Quizes</Link>
      </button>
      <button className=" px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-all duration-200"></button>
    </div>
  </div>
  if (isError)
    return (
  ...
  const handleAnswer = () => {
    ...
    console.log(
      `Selected: ${choose}, Correct: ${currentQuestion.correct_answer}`,
    )
    setAlert(
      isCorrect
        ? 'Correct answer!'
        : `Wrong answer! The correct answer was: ${currentQuestion.correct_answer}`,
    )
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className="p-7 h-[100vh] overflow-y-auto flex flex-col items-center justify-between w-full"
      >
        <h1 className="text-[18px] sm:text-2xl font-semibold mb-12 w-full text-start">
          Category: {currentQuestion.category}
        </h1>

        <div className="mb-6 p-4 flex flex-col items-center w-full justify-center ">
          <h2
            className="text-[16px] sm:text-2xl font-medium mb-2"
