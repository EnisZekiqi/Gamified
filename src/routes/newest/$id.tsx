...
function RouteComponent() {
  ...
  const [questions, setQuestions] = useState<Array<QuizData> | null>(null)
  ...
  return (
    <div className="p-7 h-[100vh] overflow-y-auto flex flex-col items-center justify-between w-full">
      ...
      <div className="mb-6 p-4 flex flex-col items-center w-full justify-center">
        ...
        <ul className="grid grid-cols-2 gap-4 list-disc list-inside items-start justify-center w-full mt-8">
          {allAnswers.map((answer, ansIndex) => (
            ...
            >
              <label className="flex items-center justify-center gap-2 cursor-pointe text-sm md:text-lg">
                <input
                  type="radio"
        ...
        {alert && (
          <div
            className={`mt-4 ${color} text-md font-light text-center w-full`}
          >
            {alert}
          </div>
        )}
      ...
      <div className="flex items-center justify-between w-full">
        ...
        >
          {indexValue < (questions ? questions.length - 1 : 0)
            ? 'Next Question'
            : 'Finish Quiz'}
        </button>
      </div>
